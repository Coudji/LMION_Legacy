...
function GaragePickup.getParcelIdentity(item)
    ...
    if state == nil or segment == nil or runtime == nil then
        return nil
    end
...
local function finalizeParcel(item, runtime, role)
    if item == nil or runtime == nil then
        return item
    end
...
local function buildPickupPlan(source, runtime)
    if source == nil or runtime == nil then
        return nil
    end

    local chain = LMION.getGarageChain(source)
    if chain == nil or chain.definitionId ~= runtime.definitionId then
        return nil
    end
...
local function canPickUpMember(previousCanPickUp, character, entry)
    ...
    local canPickUp = previousCanPickUp(
        moveProps,
        character,
        entry.square,
        entry.object
...
local function canPickUpPlan(previousCanPickUp, character, plan)
    ...
    for position = 1, plan.length do
        ...
        if not entry.object:isObjectNoContainerOrEmpty()
            or not canPickUpMember(previousCanPickUp, character, entry)
        then
            return false
        end
    end
    ...
    return inventory ~= nil
        and inventory:hasRoomFor(
            character,
            plan.runtime.weight * plan.length
        )
end
...
local function installHooks()
    ...
    ISMoveableSpriteProps.canPickUpMoveable = function(self, character, square, object)
        ...
        return canPickUpPlan(previousCanPickUp, character, plan)
    end
    ...
    ISMoveableSpriteProps.pickUpMoveableInternal = function(
        ...
        if runtime ~= nil then
            return finalizeParcel(item, runtime, self.lmionGarageRole)
        end
    ...
    ISMoveableSpriteProps.pickUpMoveable = function(
        ...
        if not forceAllow
            ...
            and not canPickUpPlan(previousCanPickUp, character, plan)
        then
            return false
        end
        ...
        -- All members have already passed the complete preflight above. From
        -- here on, let vanilla own each physical removal/item transfer exactly
        -- as it does for ordinary Moveables. One physical garage member always
        -- produces one independent generic LMION parcel.
        for position = 1, plan.length do
            local entry = plan.entries[position]
...
function GaragePickup.install()
    ...
    Events.OnGameBoot.Add(function()
        ...
        print(
            "[LMION:Pickup] garage pickup ready: "
