...
local function getSquare(anchor, offset)
    if anchor == nil or type(offset) ~= "table" then return nil end
    return getCell():getGridSquare(anchor.x + tonumber(offset[1]), anchor.y + tonumber(offset[2]), anchor.z)
end

local function getObjectSegment(object)
    if object == nil or not LMION.isDoorObject(object) then return nil end
    local sprite = object:getSprite()
    local segment = sprite and LargeGatePickup.getSegment(sprite:getName()) or nil
    if segment == nil or IsoDoor == nil or IsoDoor.getDoubleDoorIndex == nil then return nil end
...
local function findSegmentObject(runtime, square, facing, leaf, partIndex, isOpen)
    if runtime == nil or square == nil then return nil end
    local expectedSprite = LargeGatePickup.getPartSprite(runtime.definitionId, facing, leaf, partIndex, isOpen)
...
local function matchesLeaf(runtime, anchor, facing, leaf, state)
    ...
    for partIndex = 1, 2 do
        ...
        if findSegmentObject(runtime, getSquare(anchor, layout[logicalIndex]), facing, leaf, partIndex, isOpen) == nil then
            return false
        end
    end
    return true
...
local function getPartnerState(runtime, anchor, facing, leaf)
    ...
    if matchesLeaf(runtime, anchor, facing, partner, "closed") then return "closed" end
    if matchesLeaf(runtime, anchor, facing, partner, "open") then return "open" end
    return hasLeafFragment(runtime, anchor, facing, partner) and "incoherent" or "none"
end
...
local function findParcel(character, definitionId, leaf, partIndex, preferred)
    if preferred ~= nil and itemMatches(preferred, definitionId, leaf, partIndex) then
        ...
        if container ~= nil then return preferred, container end
        local worldItem = preferred.getWorldItem ~= nil and preferred:getWorldItem() or nil
        if worldItem ~= nil and worldItem:getSquare() ~= nil then return preferred, "floor" end
    end
    ...
    if items ~= nil then
        for index = 0, items:size() - 1 do
            ...
            if itemMatches(item, definitionId, leaf, partIndex) then return item, inventory end
        end
    end

    local playerSquare = character and character:getSquare() or nil
    if playerSquare == nil then return nil, nil end
            ...
            if worldObjects ~= nil then
                for index = 0, worldObjects:size() - 1 do
                    ...
                    if instanceof(worldObject, "IsoWorldInventoryObject") then
                        ...
                        if itemMatches(item, definitionId, leaf, partIndex) then return item, "floor" end
                    end
                end
            end
        end
...
local function buildPlan(moveProps, character, square, item)
    ...
    if type(selectedOffset) ~= "table" then return nil end
    ...
    if partnerState == "incoherent" then return nil end

    local targetState = partnerState == "open" and "open" or "closed"
    local layout = runtime.topology.layout[facing][targetState]
    local plan = { runtime = runtime, facing = facing, leaf = leaf, isOpen = targetState == "open" }

    for partIndex = 1, 2 do
        local logicalIndex = tonumber(indices[partIndex])
        local targetSquare = getSquare(anchor, layout[logicalIndex])
        local parcel, source = findParcel(character, runtime.definitionId, leaf, partIndex, partIndex == selectedPart and item or nil)
        local closedSprite = LargeGatePickup.getPartSprite(runtime.definitionId, facing, leaf, partIndex, false)
        ...
        if targetSquare == nil
            ...
            or not partProps:canPlaceMoveableInternal(character, targetSquare, parcel)
            or (targetState == "open" and not partProps:isFreeTile(targetSquare))
            or not LMION.canPlaceDoorAt(targetSquare, facing, false, nil)
        then
            return nil
        end
        plan[partIndex] = { square = targetSquare, item = parcel, source = source, closedSprite = closedSprite }
    end
    return plan
end
...
local function installHooks()
    ...
    ISMoveableSpriteProps.hasFaces = function(self)
        if getRuntime(self) ~= nil then return true end
    ...
    ISMoveableSpriteProps.canPlaceMoveable = function(self, character, square, item)
        if getRuntime(self) ~= nil then return buildPlan(self, character, square, item) ~= nil end
    ...
    ISMoveableSpriteProps.placeMoveable = function(self, character, square, origSpriteName, forceAllow)
        local runtime = getRuntime(self)
        if runtime == nil then return previousPlace(self, character, square, origSpriteName, forceAllow) end

        local selectedItem = findParcel(character, runtime.definitionId, self.lmionLargeGateLeaf, self.lmionLargeGatePart, nil)
        ...
        for partIndex = 1, 2 do consumeParcel(plan[partIndex].item, plan[partIndex].source) end
        return true
    end
end
