...
LMION.Build.GarageBuild = LMION.Build.GarageBuild or {}
...
--[[
This file lives in shared because both solo/server reconstruction and future MP
transport need the same hook body. Shared files are auto-executed by PZ, so this
module MUST remain inert at load time: ISBuildIsoEntity is defined in the later
server Lua tree and is not available during the initial shared phase.

The server-phase loader requires BuildingObjects/ISBuildIsoEntity first, then
calls this installer explicitly.
]]
function GarageBuild.installCursorHook()
    if GarageBuild._cursorHookInstalled == true then
        return true
    end

    if ISBuildIsoEntity == nil then
        LMION.error("Build", "garage cursor hook requested before ISBuildIsoEntity is available")
        return false
    end
    ...
    -- length as an optional `new()` parameter makes it part of the vanilla action
    -- reconstruction path instead of global mutable state.
    ...
    GarageBuild._cursorHookInstalled = true
    LMION.log("Build", "variable garage cursor hook installed")
    return true
end
